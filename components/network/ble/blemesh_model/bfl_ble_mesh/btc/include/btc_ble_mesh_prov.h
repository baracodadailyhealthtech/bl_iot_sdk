...
typedef union {
    struct ble_mesh_init_args {
        bfl_ble_mesh_prov_t *prov;
        bfl_ble_mesh_comp_t *comp;
        SemaphoreHandle_t semaphore;
    } mesh_init;
    struct ble_mesh_node_prov_enable_args {
        bfl_ble_mesh_prov_bearer_t bearers;
    } node_prov_enable;
    ...
    struct ble_mesh_set_device_name_args {
        char name[BFL_BLE_MESH_DEVICE_NAME_MAX_LEN + 1];
    } set_device_name;
    ...
    struct ble_mesh_provisioner_dev_add_args {
        bfl_ble_mesh_unprov_dev_add_t add_dev;
        bfl_ble_mesh_dev_add_flag_t   flags;
    } provisioner_dev_add;
    struct ble_mesh_provisioner_prov_dev_with_addr_args {
        uint8_t uuid[16];
        bfl_ble_mesh_bd_addr_t addr;
        bfl_ble_mesh_addr_type_t addr_type;
        bfl_ble_mesh_prov_bearer_t bearer;
        uint16_t oob_info;
        uint16_t unicast_addr;
    } provisioner_prov_dev_with_addr;
    struct ble_mesh_provisioner_dev_del_args {
        bfl_ble_mesh_device_delete_t del_dev;
    } provisioner_dev_del;
    ...
    struct ble_mesh_provisioner_set_prov_net_idx_args {
        bfl_ble_mesh_prov_data_info_t prov_data;
    } set_prov_data_info;
    ...
    struct ble_mesh_provisioner_set_node_name_args {
        uint16_t index;
        char name[BFL_BLE_MESH_NODE_NAME_MAX_LEN + 1];
    } set_node_name;
    ...
    struct ble_mesh_start_ble_advertising_args {
        bfl_ble_mesh_ble_adv_param_t param;
        bfl_ble_mesh_ble_adv_data_t  data;
    ...
    struct ble_mesh_deinit_args {
        bfl_ble_mesh_deinit_param_t param;
    } mesh_deinit;
...
bfl_ble_mesh_model_t *btc_ble_mesh_model_find(const bfl_ble_mesh_elem_t *elem,
        uint16_t id);
