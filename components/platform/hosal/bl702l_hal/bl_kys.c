...
static void bl_kys_gpio_init(uint8_t row_num, uint8_t col_num, uint8_t row_pins[], uint8_t col_pins[])
{
...
static void bl_kys_irq(void)
{
    ...
    for(i=0; i<result.key_num; i++){
        key_code = KYS_ReadKeyfifo();
        result.row_idx[i] = key_code % row_num;
        result.col_idx[i] = key_code / col_num;
    }

    KYS_IntClear(KYS_INT_CLR_ALL);
    KYS_Disable();
...
void bl_kys_init(uint8_t row_num, uint8_t col_num, uint8_t row_pins[], uint8_t col_pins[])
{
    ...
    // Run KYS at 2M (better no greater than 4M)
    GLB_PER_Clock_UnGate(GLB_AHB_CLOCK_KYS);
    GLB_Set_KYS_CLK(GLB_KYS_CLK_SRC_XCLK, 15);
